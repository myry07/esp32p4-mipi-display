...
static esp_err_t build_jpg_list(const char *path, char ***out_list, int *out_n)
{
    *out_list = NULL; *out_n = 0;
    ...
    if (cnt==0) { closedir(dir); ALBUM_LOG("no jpg in %s", path); return ESP_FAIL; }
...
{
    if (!ensure_canvas(c)) return false;

    // 读文件
    FILE *fp = fopen(path, "rb");
    if (!fp) { ALBUM_LOG("open %s fail", path); return false; }
    fseek(fp, 0, SEEK_END);
    long fsz = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    if (fsz <= 0) { fclose(fp); ALBUM_LOG("size 0"); return false; }

    uint8_t *jpg = (uint8_t*)safe_calloc_align((size_t)fsz, JPEG_ALIGN);
    if (!jpg) { fclose(fp); ALBUM_LOG("no mem jpg"); return false; }
    size_t rd = fread(jpg, 1, (size_t)fsz, fp);
    fclose(fp);
    if (rd != (size_t)fsz) { safe_free_align(jpg); ALBUM_LOG("read fail"); return false; }
    ...
    if (!c->j) {
        ...
        if (jpeg_dec_open(&cfg, &c->j) != JPEG_ERR_OK) {
            safe_free_align(jpg); ALBUM_LOG("jpeg open fail"); return false;
        }
    }
    ...
    if (jpeg_dec_parse_header(c->j, &io, &hi) != JPEG_ERR_OK) {
        safe_free_align(jpg); ALBUM_LOG("parse hdr fail"); return false;
    }

    int out_len = 0;
    if (jpeg_dec_get_outbuf_len(c->j, &out_len) != JPEG_ERR_OK || out_len <= 0) {
        safe_free_align(jpg); ALBUM_LOG("get out len fail"); return false;
    }

    // 动态保证解码缓冲容量
    if (out_len > c->decode_cap) {
        if (c->decode_buf) { safe_free_align(c->decode_buf); c->decode_buf = NULL; }
        c->decode_buf = (uint8_t*)safe_calloc_align((size_t)out_len, JPEG_ALIGN);
        if (!c->decode_buf) { safe_free_align(jpg); ALBUM_LOG("no mem decode buf %d", out_len); return false; }
...
static void album_event_cb(lv_event_t *e)
{
    ...
    case LV_EVENT_PRESSED: {
        ...
        if (indev) lv_indev_get_point(indev, &c->p_down);
    ...
    case LV_EVENT_RELEASED: {
        ...
        if (c->count <= 0 || !c->paths) break;   // 没有图片就不处理

        lv_indev_t *indev = lv_indev_get_act();
        if (!indev) break;

        lv_point_t p_up = {0};
        lv_indev_get_point(indev, &p_up);

        int dx = p_up.x - c->p_down.x;
        int dy = p_up.y - c->p_down.y;

        // —— 自适应阈值：取 SWIPE_THRESHOLD_PX、屏幕宽度的5%、DPI/8 三者最大 ——
        int thr = SWIPE_THRESHOLD_PX;                       // 你已有的常量（例如 15~20）
        lv_obj_t *target = lv_event_get_current_target(e);  // 事件绑定的对象（页面/容器）
        if (target) {
            int w = lv_obj_get_width(target);
            int thr_w = w / 20;        // 5% 宽度
            if (thr_w > thr) thr = thr_w;
        }
        uint32_t dpi = lv_disp_get_dpi(NULL);
        if (dpi) {
            int thr_dpi = (int)(dpi / 8);   // 约 12~20 px
            if (thr_dpi > thr) thr = thr_dpi;
        }
        if (thr < 12) thr = 12;             // 合理下限
        if (thr > 60) thr = 60;             // 避免过大

        // 快速短划（从按下到抬起 < 160ms） → 放宽阈值（2/3）
        uint32_t dur = lv_tick_elaps(s_press_tick);
        if (dur < 160) thr = (thr * 2) / 3;

        // 必须明显更“水平”，避免斜划/竖划被误判
        // 5*|dx| < 6*|dy| 约等价于 |dx| < 1.2*|dy|（整数比较，不卡浮点）
        if (abs(dx) * 5 < abs(dy) * 6) break;
